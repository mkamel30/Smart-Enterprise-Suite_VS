...
async function stockIn(req, { partId, quantity, reason, branchId }) {
  ...
  try {
    await db.stockMovement.create({ data: { branchId: bId, partId, type: 'IN', quantity, reason: reason || 'ÊÍæíá ãä ÇáÅÏÇÑÉ', createdAt: new Date(), userId: req.user?.id, performedBy: req.user?.displayName || 'System' } });
...
async function importStock(req, items, branchId) {
  ...
  for (const item of items) {
    ...
    try {
      await db.stockMovement.create({ data: { branchId: bId, partId: item.partId, type: 'IN', quantity: item.quantity, reason: 'ÇÓÊíÑÇÏ ãä ãáÝ Excel', createdAt: new Date(), performedBy: req.user?.displayName || 'System' } });
    } catch (e) { }
    updated++;
  }
  return { updated };
...
async function stockOut(req, { partId, quantity, reason, requestId, cost, customerId, customerName, userId, userName }, branchId) {
  ...
  if (!inventoryItem || inventoryItem.quantity < quantity) throw new Error('ÇáßãíÉ ÛíÑ ßÇÝíÉ');
  ...
  const result = await db.$transaction(async (tx) => {
    ...
    if (cost && cost > 0 && customerId) {
      payment = await tx.payment.create({ data: { branchId: bId, customerId, customerName, requestId, amount: roundMoney(cost), type: 'MAINTENANCE', reason: 'ÞØÚ ÛíÇÑ ÕíÇäÉ', paymentPlace: 'ÖÇãä', notes: reason || 'ÞØÚ ÛíÇÑ ÕíÇäÉ', userId, userName, receiptNumber: req.body.receiptNumber } });
    }
...
const { validateBranches } = require('../../../utils/transferValidator');
...
async function transferStock(req, { partId, quantity, fromBranchId, toBranchId, reason }) {
  ...
  await db.$transaction(async (tx) => {
    // Apply "The Binding Law" (ÇáÞÇäæä ÇáãáÒã)
...
async function getMovements(req) {
  ...
  let results = movements.map(m => {
    ...
    return {
      ...
      partName: partMap[m.partId]?.name || 'ÛíÑ ãÚÑæÝ',
...
async function deductParts(parts, requestId, performedBy, branchId, receiptNumber = null, tx = null) {
  ...
  for (const part of parts) {
    ...
    if (!invItem) {
      throw new Error(`ÇáÞØÚÉ "${part.name}" ÛíÑ ãæÌæÏÉ Ýí ÇáãÎÒä`);
    }

    // 2. VALIDATE quantity
    if (invItem.quantity < part.quantity) {
      throw new Error(
        `ÇáßãíÉ ÇáãÊÇÍÉ ãä "${part.name}" ÛíÑ ßÇÝíÉ. ` +
        `ãÊæÝÑ: ${invItem.quantity}¡ ãØáæÈ: ${part.quantity}`
      );
    }
    ...
    if (newQuantity <= (invItem.minLevel || 5)) {
      socketManager.emitToBranch(branchId, 'stock-alert', {
        ...
        message: `ÊäÈíå: ãÎÒæä "${part.name}" æÕá ááÍÏ ÇáÃÏäì (${newQuantity})`
      });
    }
    ...
    const movement = await txOrDb.stockMovement.create({
      data: {
        ...
        reason: part.reason || 'ÞØÚ ÛíÇÑ ÕíÇäÉ',
        requestId: requestId,
