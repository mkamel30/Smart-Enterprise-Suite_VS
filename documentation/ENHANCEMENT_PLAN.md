...
1. **PostgreSQL Migration**:
   ...
   - *Action*: Update Prisma configuration for PostgreSQL and prepare data migration scripts.
...
1. **Split monolithic files**:
   - `maintenanceCenterService.js` (1595 lines) → inspection, repair, approval sub-services
   - `api/client.ts` (1518 lines) → domain-specific API modules (`authApi.ts`, `warehouseApi.ts`, etc.)
   - Large route files → sub-routers
...
6. **Split large frontend page components** (`ExecutiveDashboard.tsx`, `MaintenanceMachineDetail.tsx`, `Dashboard.tsx`) into sub-components.
