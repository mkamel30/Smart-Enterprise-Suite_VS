...
echo [3/4] Stopping PostgreSQL container...
docker-compose stop
if %errorlevel%==0 (
    echo       OK - Database container stopped
) else (
    echo       -- Database was not running or docker-compose failed
)

echo.
echo [4/4] Freeing ports 5002 and 5173...
...
netstat -aon | find ":5002" | find "LISTENING" >nul 2>&1
if %errorlevel%==0 (
    echo       WARNING: Port 5002 still occupied
    set CLEAN=0
) else (
    echo       OK - Port 5002 is free
)
