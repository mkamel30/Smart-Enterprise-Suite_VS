...
### 8. 📦 adminStoreService.js - خدمة مخزن الشئون الإدارية

**المسار:** `backend/services/adminStoreService.js`

**الوصف:** إدارة مركزية لمخزون الأصول الجديدة (ماكينات، شرائح، أصول إدارية) وتوزيعها على الفروع.

| Function | الوصف | Parameters | Returns |
|----------|-------|------------|---------|
| `listItemTypes(filters)` | جلب أنواع الأصناف | `{category?, isActive?, search?}` | `Promise<AdminStoreItemType[]>` |
| `createItemType(data, user)` | إنشاء نوع صنف جديد | `{code, name, category, trackingMode, ...}` | `Promise<AdminStoreItemType>` |
| `listAssets(filters)` | جلب قائمة الأصناف | `{itemTypeCode?, status?, branchId?, search?}` | `Promise<AdminStoreAsset[]>` |
| `createAsset(data, user)` | إنشاء صنف مفرد | `{itemTypeCode, serialNumber, ...}` | `Promise<AdminStoreAsset>` |
| `createCarton(data, user)` | إنشاء كرتونة جديدة | `{cartonCode, itemTypeCode, machinesCount, ...}` | `Promise<AdminStoreCarton>` |
| `transferAsset(assetId, toBranchId, notes, user)` | تحويل صنف مفرد (مع حماية ذرية) | انظر التفاصيل | `Promise<TransferOrder>` |
| `transferCarton(cartonId, toBranchId, notes, user)` | تحويل كرتونة كاملة (مع حماية ذرية) | انظر التفاصيل | `Promise<TransferOrder>` |
| `bulkTransferAssetsAndCartons(data, user)` | تحويل جماعي (مع حماية ذرية) | `{assetIds?, cartonCodes?, toBranchId, notes?}` | `Promise<TransferOrder>` |
| `getAssetHistory(assetId)` | سجل حركات صنف | `assetId: string` | `Promise<AdminStoreMovement[]>` |

#### 🛡️ نظام الحماية من التكرار (Race Condition Protection):

جميع وظائف التحويل (`transferAsset`, `transferCarton`, `bulkTransferAssetsAndCartons`) تطبق **التحقق الذري** داخل Database Transaction:
...
تم تصحيح عرض بيانات الشرائح في أذون التحويل (v3.5.1):
...
**📚 للمزيد:** راجع [ADMIN_STORE_REFERENCE.md](./ADMIN_STORE_REFERENCE.md) للتوثيق الكامل.
