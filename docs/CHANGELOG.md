...
## [3.5.1] - 2026-02-18
### Fixed
- **🛡️ Admin Store - Transfer Security**:
  - Implemented atomic status validation inside DB transactions to prevent duplicate transfer orders (Race Condition Protection).
  - Added specific error messages for already-transferred items.
- **🏷️ Admin Store - SIM Data Integrity**:
  - Fixed mapping of SIM card attributes during transfer: `Network Type` now maps to `Model` (النوع), and `Provider` now maps to `Manufacturer` (المصنع).
  - Removed "Administrative Affairs" placeholder from SIM manufacturer field in transfer orders.
- **🎨 Admin Store - UI UX Fixes**:
  - Resolved "Duplicate Key" console errors in the Create Carton modal when processing serial numbers that don't end in digits.
  - Improved serial number preview logic in the manual carton creation tool.

### Maintenance
- **🧹 Data Correction**:
  - Cleaned up duplicate transfer orders and orphan movement logs caused by previous race conditions.
  - Corrected live data for pending Transfer Order `TO-69475825-383`.

## [3.5.0] - 2026-02-13
### Added
- **⚖️ The "Binding Law" (القانون الملزم) for Internal Transfers**:
  - Implemented strict business rules for all item transfers (Machines, SIMs, Assets, Parts) across the organization.
  - **Rule 1 (Branch → Admin Affairs)**: Restricts transfers to Machines and SIM cards only.
  - **Rule 2 (Admin Affairs → Branch)**: Full distribution permission for all item types.
  - **Rule 3 (Branch → Maintenance Center)**: Restricts transfers to Machines only and enforces assigned center mapping.
  - **Rule 4 (Branch <-> Branch)**: Requires a direct hierarchy relationship (Parent-Child) for any direct branch-to-branch transfers.
- **🛡️ Multi-Layer Validation**:
  - Integrated the Binding Law into `TransferOrderService`, `AdminStoreService`, and `InventoryService`.
  - Added backend validation in `transfer-validators.js` to ensure data integrity.

### Changed
- **Branch Selection Logic**:
  - Updated `AdminAssetTransferModal` to filter and show only compatible target branches based on the new rules.
  - Allowed `ADMIN_AFFAIRS` role to see all valid branches for transfer distribution by updating the `branches/authorized` endpoint.

### Fixed
- **UI UX Refinement**:
  - Improved the styling and clarity of branch selection dropdowns in administrative modals.
  - Fixed various display issues and text overlaps in the Transfer modals.
### Security (Critical)
