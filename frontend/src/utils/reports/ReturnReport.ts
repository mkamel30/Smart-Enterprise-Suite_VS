...
export function generateReturnReport(props: ReturnReportProps): string {
    const { customer, machine, reason, notes } = props;
    const dateStr = new Date().toLocaleDateString('ar-EG');
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
    const filename = `Return_${customer.bkcode || 'Client'}_${timestamp}`;
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .content { display: flex; flex-wrap: wrap; gap: 20px; margin-bottom: 20px; }
    ...
    <div class="sheet">
        <div class="header">
            ...
            <div class="header-center">
                <h1>إيصال استرجاع ماكينة للمخزن</h1>
            </div>
            <div class="logo">
        ...
        <div class="content">
            <div class="col">
                <div class="field">
                    <span class="label">اسم العملاء:</span>
                    <span class="value">${customer.client_name}</span>
                </div>
                <div class="field">
                    <span class="label">كود العميل:</span>
                    <span class="value">${customer.bkcode}</span>
                </div>
            </div>
            <div class="col">
                <div class="field">
                    <span class="label">السيريال:</span>
                    <span class="value" style="font-family: monospace; font-size: 1.1em;">${machine.serialNumber}</span>
                </div>
                 <div class="field">
        ...
        <div class="field" style="width: 100%;">
            <span class="label" style="width: auto; margin-left: 10px;">سبب الاسترجاع والملاحظات:</span>
            <div class="notes-box">
...
export function openReturnReport(props: ReturnReportProps) {
    const html = generateReturnReport(props);
